...
function extractValue(arr, key) {
    return arr.reduce(function(total, nextVal){
        total.push(nextVal[key]);                
        return total;
    }, []);
}
...
function vowelCount(str) {
    const vowels = 'aeiou';
    return str.split('').reduce(function(total, nextVal){
        let lowerCased = nextVal.toLowerCase();
        if (vowels.indexOf(lowerCased) !== -1){
            total[lowerCased]? total[lowerCased]++ : total[lowerCased] = 1;  
            }
        return total;
    }, {});
}
...
accepts an array of objects and 
...
function addKeyAndValue(arr, key, value) {
    return arr.reduce(function(total, nextVal, currentIndex){
        total[currentIndex][key] = value; 
        return total;
    }, arr);
}
...
if the result of the callback function at that specific value is true, the 
value should be placed in the first subarray. 
...
function partition(arr, callback) {
    arr.reduce(function(total, nextVal){
        callback(nextVal) ? total[0].push.nextVal : total[1].push.nextVal;
        return total;
    },[[],[]])
}
